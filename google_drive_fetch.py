"""
Google Drive Integration Module - PiffTok Lens
...
def get_drive_service():
    try:
        ...
        if os.path.exists(GOOGLE_SERVICE_ACCOUNT_KEY):
            creds = service_account.Credentials.from_service_account_file(GOOGLE_SERVICE_ACCOUNT_KEY, scopes=SCOPES)
...
def list_drive_videos(service):
    ...
    try:
        ...
        for mime_type in video_mime_types:
            ...
            results = service.files().list(q=query_with_mime, fields="files(id, name, size, mimeType)", spaces='drive').execute()
...
def fetch_one_video_from_drive(allow_repost=False):
    ...
    print("=" * 60)
    print("FETCHING VIDEO FROM GOOGLE DRIVE - PIFFTOK LENS")
    print("=" * 60)
    ...
    if all_are_published:
        if not allow_repost:
            ...
            return None
        print("\nREPOST MODE: Selecting random published video...")
